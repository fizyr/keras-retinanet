...
def parse_args(args):
    ...
    parser.add_argument('--nms-threshold', help='Value for non maximum suppression threshold', dest='nms_threshold', type=float, default=0.5)
    parser.add_argument('--score-threshold', help='Threshold for prefiltering boxes', dest='score_threshold', type=float, default=0.05)
    parser.add_argument('--max-detections', help='Maximum number of detections to keep', dest='max_detections', type=int, default=300)
    parser.add_argument('--parallel-iterations', help='Number of batch items to process in parallel.', dest='parallel_iterations', type=int, default=32)
...
def main(args=None):
    ...
    model = models.convert_model(model, nms=args.nms, class_specific_filter=args.class_specific_filter, anchor_params=anchor_parameters,
                                nms_threshold=args.nms_threshold, score_threshold=args.score_threshold, max_detections=args.max_detections,
